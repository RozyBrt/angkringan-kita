...
interface AnalyticsClientProps {
  initialRevenueData: { date: string; revenue: number; orders: number }[];
  initialTopItems: { name: string; value: number }[];
}
...
export default function AnalyticsClient({ initialRevenueData, initialTopItems }: AnalyticsClientProps) {
  ...
  async function handleDayChange(newDays: number) {
    setDays(newDays);
    setLoading(true);
    const [revRes, topRes] = await Promise.all([
      getRevenueStats(newDays),
      getTopItems(newDays)
    ]);
      ...
      <div className="max-w-6xl mx-auto mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        ...
        <div className="flex items-center gap-3 bg-coffee-900/60 p-1.5 rounded-2xl border border-coffee-800/50 backdrop-blur-md">
          {timeRanges.map((range) => (
            <button
              key={range.value}
              onClick={() => handleDayChange(range.value)}
              disabled={loading}
